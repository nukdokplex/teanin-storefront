...
export default async function Page(props: { params: Promise<{ channel: string }> }) {
	...
	if (!checkout || checkout.lines.length < 1) {
		return (
          ...
          Вы пока ничего ещё не добавили в корзину
				</p>
				<LinkWithChannel
					href="/products"
								...
								<div className="flex justify-between justify-items-start gap-4">
									<div>
										...
										{item.variant.name !== item.variant.id && Boolean(item.variant.name) && (
											<p className="mt-1 text-sm text-neutral-500">Вариант: {item.variant.name}</p>
										)}
									</div>
				...
				<div className="mt-12">
					<div className="rounded border bg-neutral-50 px-4 py-2">
						<div className="flex items-center justify-between gap-2 py-2">
							<div>
								<p className="font-semibold text-neutral-900">Итого</p>
								<p className="mt-1 text-sm text-neutral-500">Цена доставки будет рассчитана на следующем шаге</p>
							</div>
